...
bool GasNetwork::can_be_paired(int start_id, int stop_id) {
    ...
    for (Connection con : connections)
        if ((con.start_id == start_id && con.stop_id == stop_id) || (con.start_id == stop_id && con.stop_id == start_id)) 
            return false;
    return true;
}
...
void GasNetwork::EraseConnections_byCS(int station_id) {
    int x = 0;
    for (Connection con : connections) {
...
void GasNetwork::NewCS() {
    CS newCS;
    cin >> newCS;
    stations[stations_id] = newCS;
    stations_id++;
    system("clear");
}
...
void GasNetwork::EditPipes(vector<int> selectedPipes) {
    ...
    case 3:
        ...
        d = Enter<int>();
        while (d <= 0) {
            cout << "Диаметр должен превышать 0\nПовторите ввод: ";
            d = Enter<int>();
        }
        for (const auto& x : selectedPipes)
...
void GasNetwork::EditStations(vector<int> selectedStations) {
    ...
    case 2:
        ...
        w = Enter<int>();
        while (w < 0 || w < c) {
            cout << "Число должно быть положительным и больше " << c << "\nПовторите ввод: ";
            w = Enter<int>();
        }
        for (const auto& x : selectedStations)
    ...
    case 3:
        ...
        w = Enter<int>();
        while (w > c || w < 0) {
            cout << "Количество не должно превышать общее количество цехов. (0 <= count <= " << c << ")\nПовторите ввод: ";
            w = Enter<int>();
        }
        for (const auto& x : selectedStations)
...
void GasNetwork::LoadData() {
    ...
    while (getline(inFile, line)) {
        if (line == "#PIPE") {
            ...
            getline(inFile, type);
            stations[id] = CS(name, workshops_count, workshops_working, type);
...
void GasNetwork::ShowTopo() {
    ...
    if (topoList.empty()) 
        cout << "У вас нет КС!";
    else if (topoList[0][0] == -1) 
        cout << "Сортировка невозможна - граф имеет цикл!";
    else {
